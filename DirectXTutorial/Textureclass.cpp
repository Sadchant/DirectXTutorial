...
// The Initialize functions take as input the Direct3D device and the name of the targa image file.It will first load the targa data into an array.
...
bool TextureClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, char* filename)
{
	...
	D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc; // Describes a shader-resource view.
	...
	// Next we need to setup our description of the DirectX texture that we will load the targa data into. We use the height and width from the targa image data, and set the format to be
	// a 32 bit RGBA texture. We set the SampleDesc to default. Then we set the Usage to D3D11_USAGE_DEFAULT which is the better performing memory, which we will also explain a bit more
	// about down below. And finally we set the MipLevels, BindFlags, and MiscFlags to the settings required for Mipmaped textures. Once the description is complete we call CreateTexture2D
	...
	// If you will remember from the previous tutorial we used Map and Unmap to copy our matrices in the ModelClass
	// into the matrix constant buffer, and we could have done the same here with our texture data. And in fact
	// using Map and Unmap is generally a lot quicker than using UpdateSubresource, however both loading methods
	// have specific purposes and you need to choose correctly which one to use for performance reasons. The
	// recommendation is that you use Map and Unmap for data that is going to be reloaded each frame or on a 
	// very regular basis. And you should use UpdateSubresource for something that will be loaded once or that gets
	// loaded rarely during loading sequences. The reason being is that UpdateSubresource puts the data into higher
	// speed memory that gets cache retention preference since it knows you aren't going to remove or reload it anytime
	...
	// Mipmap levels for high quality texture rendering at any distance. Once the shader resource view is created we call
	// GenerateMips and it creates the Mipmaps for us, however if you want you can load your own Mipmap levels in manually
	// if you are looking for even better quality.
